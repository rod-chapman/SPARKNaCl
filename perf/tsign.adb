...
--      16#82#, 16#82#, 16#D2#, 16#D0#, 16#93#, 16#6C#, 16#19#, 16#10#);
   ...
   procedure Tweet_Open (M      :    out Byte_Seq;
                         Status :    out Boolean;
                         MLen   :    out I32;
                         SM     : in     Byte_Seq;
                         PK     : in     Signing_PK);
   ...
   is
      ...
      LMLen : Unsigned_64;
   begin
      R := TweetNaCl_API.Crypto_Sign_Open (M, LMLen, SM, SM'Length, PK);
      Status := (R = 0);
      MLen := I32 (LMLen);
   end Tweet_Open;
...
begin
   ...
   SPARKNaCl.Sign.Keypair (Raw_SK, PK, SK);
--   Construct (My_SK, SK);
   ...
   IO.Put_Line ("SPARKNaCl.Open test");
   T1 := Mcycle.Read;
   SPARKNaCl.Sign.Open (M2, Open_OK, M2Len, SM1, PK);
   T2 := Mcycle.Read;
   Total_Time := Unsigned_64 (T2 - T1);
   Report;
   IO.Put_Line (if Open_OK then "Open OK" else "Open FAIL");
   ...
   IO.Put_Line ("TweetNaCl.Open test");
   T1 := Mcycle.Read;
   Tweet_Open (M3, Open_OK, M3Len, SM2, PK);
   T2 := Mcycle.Read;
   Total_Time := Unsigned_64 (T2 - T1);
   Report;
   IO.Put_Line (if Open_OK then "Open OK" else "Open FAIL");
